...
async def check_afk(war_id):
    ...
    for user in war['users']:
        ...
        if updated_at < (datetime.datetime.now() - datetime.timedelta(minutes=5, hours=3)):
            tg_user = json.loads(
            ...
            await bot.send_message(chat_id=admins[0],
                                   text=f"{updated_at}, {datetime.datetime.now()}, {updated_at < (datetime.datetime.now() - datetime.timedelta(minutes=5, hours=3))}")
